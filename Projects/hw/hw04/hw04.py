...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m)
        mobile_left, mobile_right = left(m), right(m)
        return length(mobile_left) * total_weight(end(mobile_left)) == length(mobile_right) * total_weight(end(mobile_right)) \
               and balanced(end(mobile_left)) and balanced(end(mobile_right))
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m)
        return tree(total_weight(m), tree(totals_tree(end(left(m)))) + tree(totals_tree(end(right(m)))))
