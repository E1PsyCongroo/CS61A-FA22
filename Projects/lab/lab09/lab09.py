...
def subseqs(s):
    ...
    if not s:
        ...
        sublist = subseqs(s[1:])
        return insert_into_all(s[0], sublist) + sublist
...
def long_paths(t, n):
    ...
    result = []
    for b in t.branches:
        for ls in long_paths(b, n-1):
            result.append([t.label] + ls)
    return result
...
def reverse_other(t):
    ...
    for i in range(length//2):
        t.branches[i].label, t.branches[length-i-1].label = t.branches[length-i-1].label, t.branches[i].label
