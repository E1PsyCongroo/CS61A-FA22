...
def about(topic):
    ...
    def check_about(ls):
        ls = split(ls)
        for i in ls:
            if remove_punctuation(lower(i)) in topic:
                return True
        return False
    return check_about
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        ls = [word for word in word_list if diff_function(typed_word, word, limit) <= limit]
        if len(ls) == 0:
            return typed_word
        else:
            return min(ls, key=lambda x: diff_function(typed_word, x, limit))
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) > len(source):
        length = len(typed) - len(source)
        return length + feline_fixes(typed[:len(source)], source, limit) if length <= limit else length
    elif len(typed) < len(source):
        length = len(source) - len(typed)
        return length + feline_fixes(typed, source[:len(typed)], limit) if length <= limit else length
    else:
        if limit < 0 or len(typed) == 0:
            return 0
...
def final_diff(typed, source, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        add = minimum_mewtations(source[0] + typed, source, limit - 1)  # Fill in these lines
...
def report_progress(typed, prompt, user_id, upload):
    ...
    while i < len(typed) and typed[i] == prompt[i]:
        count += 1
        i += 1
    dt = {'id': user_id, 'progress': count/len(prompt)}
    upload(dt)
    return dt['progress']
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        ...
        for index in range(len(i)-1):
            list1 += [i[index+1]-i[index]]
        list0 += [list1]
    return {'words': words, 'times': list0}
    # END PROBLEM 9
