...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not self.is_double and hasattr(self, "damage"):
            self.damage *= 2
            self.is_double = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        place_range = 0
        while not place.is_hive and ((len(place.bees) == 0 and place_range <= self.upper_bound)
                                     or place_range < self.lower_bound):
            place = place.entrance
            place_range += 1
        if place.is_hive or place_range > self.upper_bound:
            return None
        else:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.have_QueenAnt:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            ...
            self.time += 1
            if self.time == 5:
                target.action = lambda gamestate: Bee.action(target, gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant.is_container:
            dic[self.place.ant_container] = self.place
        else:
            ...
            while not place.is_hive:
                ...
                if place.ant:
                    ...
                    if place.ant.is_container:
                        dic[place.ant.ant_contained] = place
                place = place.entrance
        return dic
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        dist = 0
        while distance != self.place:
            distance = distance.exit
            dist += 1
        damage = self.damage - dist * 0.25 - 0.0625 * self.insects_shot
